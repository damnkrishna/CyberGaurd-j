...
A real security engagement platform — or any responsible one — would need something much stronger before authorizing scans against a whole domain: proof of domain control (like a DNS TXT record challenge), a company registration check, an admin/invite-based signup flow, or a signed scope-of-work document from someone with actual organizational authority (IT head, CISO, legal). "I can receive email at this domain" is a weak proxy for that, especially for a college, where thousands of students and staff have @college.edu addresses and approximately none of them have the authority to greenlight a pentest of the institution.

So where does that leave you? There's a small sliver of "you did select 'company' knowing you're a student, not a company rep" — but the burden of verifying that claim belongs to the platform, not you. You weren't shown a consent/authorization document to sign, weren't asked "are you authorized to represent this organization," weren't warned this would trigger scanning against the whole domain and pull third-party PII. You just verified an email like any onboarding flow asks you to. The platform is the one that decided that action was sufficient authorization to scan an entire institution and expose staff data — that's their authorization logic being broken, not you exploiting anything maliciously.

If anything, this makes your finding stronger for a writeup: it's not "a sophisticated attacker bypassed security," it's "a completely ordinary signup flow, done exactly as intended, was enough to trigger unauthorized-adjacent scanning and third-party data exposure." That's a more serious design flaw precisely because it required zero malicious effort.
